...
class YOLOLoss(nn.Module):    
    def __init__(self, num_classes, fp16, strides=[8, 16, 32,4]):
        super().__init__()
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            cls_preds = (cls_preds.float().view(-1,self.num_classes).sigmoid() * obj_preds.float().view(-1,1).sigmoid().repeat(1,self.num_classes))
            loss_obj = F.binary_cross_entropy(cls_preds[~fg_masks],torch.zeros_like(cls_preds[~fg_masks]),reduction="none").sum()
            loss_cls = F.binary_cross_entropy(cls_preds[fg_masks],cls_targets,reduction="none").sum()
        reg_weight  = 5.0
    ...
    def get_in_boxes_info(self, gt_bboxes_per_image, expanded_strides, x_shifts, y_shifts, total_num_anchors, num_gt, center_radius = 2.5):
        ...
        # ---------------------------------------------更改----------------------------------------------------------------------#
        #      思路：1.创建2个tensor:第一个[gt_obj_num,8400]----->每一个行代表着这个真实框的(w,h)的最大值
        #                          第二个[gt_obj_num,8400]----->每一行代表着这个预测特征层的缩放比
        #           2.2个tensor对应位置相减得到一个新tensor[gt_obj_num,8400]--->如果gt_obj_num*8400都大于0则
        #             所有目标在三个预测特征层都可以进行预测都可以进行预测，如果有一处小于0.则不用这个网格对该目标进行预测，相应位置置为false
        #           3.根据上面求得对is_in_boxes进行进一步判断
        # ---------------------------------------------更改---------------------------------------------------------------------#
        # 第一个tensor
        # gt_bboxes_per_image_wh = gt_bboxes_per_image[:, 2:]
        # gt_bboxes_per_image_wh_max, _ = gt_bboxes_per_image_wh.topk(1)
        # gt_bboxes_per_image_wh_max_reapeat = gt_bboxes_per_image_wh_max.repeat(1, 8400)
        # # 第二个tensor
        # radio_one = torch.ones(6400)
        # radio_four = torch.ones(1600) * 4
        # radio_five = torch.ones(400) * 5
        # radio = torch.cat((radio_one, radio_four, radio_five), 0).unsqueeze(0).repeat(num_gt, 1).cuda()
        # refrence_size = torch.mul(expanded_strides, radio)
        # # 两个tensor进行相减
        # is_in_boxes_size = (gt_bboxes_per_image_wh_max_reapeat - refrence_size) > 0.0
        # # 根据上面求得对is_in_boxes进行进一步判断
        # is_in_boxes = is_in_boxes & is_in_boxes_size
        # ---------------------------------------------更改完成---------------------------------------------------------------------#
        ...
        #           3.根据上面求得对is_in_boxes进行进一步判断
        # ---------------------------------------------更改---------------------------------------------------------------------#
        # is_in_centers = is_in_centers & is_in_boxes_size
        # # ---------------------------------------------更改完成---------------------------------------------------------------------#
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        for gt_idx in range(num_gt):
            ...
            #   给每个真实框选取最小的动态k个点
            #------------------------------------------------------------#
            if(cost[gt_idx].shape[0] > dynamic_ks[gt_idx].item()):
